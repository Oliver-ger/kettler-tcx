...
class tcx:
    ...
    def __init__(self):
        ...
        self.trackpoints=[]
        self.newdistance = 0.0  # accumulated newdistance is calculated each trackpoint
        self.distance_trackpoint = 0.0 # distance between 2 trackpoints
    ...
    def add_activity(self,start=None):
        ...
        self.activity=etree.SubElement(self.activities, "Activity", Sport="Biking") #vorher: Other
...
#<xsd:element name="DistanceMeters" type="xsd:double" minOccurs="0"/>
        if "DistanceMeters" in point:
            # Berechnung bisherige Gesamtdistanz newdistance s = v * t
            if "TimeDiff" in point:
                self.distance_trackpoint = point["TimeDiff"] * point["Speed"] #distance since last trackpoint
            else:
                self.distance_trackpoint = 5.0 * point["Speed"] #distance since last trackpoint with t = 5s
            self.newdistance = self.newdistance + self.distance_trackpoint #accumulated distance
            # bisherige Gesamtdistanz in Trackpoint setzen 
            etree.SubElement(self.trackpoints[-1], "DistanceMeters").text=str(self.newdistance)
            # bisherige Gesamtdistanz in Activity setzen
...
##Lap Related:
        # bis jetzt verbrauchte Kalorien in Activity setzen
        if "Calories" in point:
            self.calories.text=str(point["Calories"])
        # bis jetzt vergangene Zeit (von runtime) in Activity setzen
